...
logger = create_logger('translate_rawtag.log')
os.environ["CUDA_VISIBLE_DEVICES"] = "0"

parser = argparse.ArgumentParser(description='Settings')
parser.add_argument("--train_data", type=str, default='data/rawtag.bin',
                    help="train data dir")
...
parser.add_argument("--checkpoint_dir", type=str, default='all_models/rawtag')
params = parser.parse_args()
params.gpu_num = 1
params.seed = 1234
params.reload_model = '{}/model_epoch{}.pt'.format(params.checkpoint_dir, params.id)
params.translate_file = 'test_nist/nist06.cn.bpe.rawtag'
#params.translate_file = 'test_nist/copy-nocopy/rawtag.nocopy'
params.src_dico_file = 'data/dict.rawtag.bpe.cn'
params.tgt_dico_file = 'data/dict.rawtag.bpe.en'
    ...
    with torch.no_grad():
        for (sen1, len1) in iterator:
            ...
            for j in range(len2.size(0)):
                file.write(params.tgt_dico.idx2string(sent2[:, j]).replace('@@ ', '').replace('<start> ','').replace('<end> ','').replace(' <end>','')+'\n')
