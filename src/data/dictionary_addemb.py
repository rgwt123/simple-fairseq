...
class Dictionary(object):
    ...
    @staticmethod
    def index_data(src_txt_path, tgt_txt_path, src_dico, tgt_dico, bin_path, src_tag_path):
        """
        ...
        fstag = open(src_tag_path, 'r', encoding='utf-8')
        for i, line in enumerate(fs):
            line_t = ft.readline()
            line_stag = fstag.readline()
            if i % 1000000 == 0 and i > 0:
                print(i)
            s = line.rstrip().split()
            s_t = line_t.rstrip().split()
            s_stag = line_stag.rstrip().split()
            # skip empty sentences
            if (len(s) == 0) or (len(s_t) == 0 or len(s_stag) == 0):
            ...
            sentences_stag.extend(indexed_stag)
            sentences_stag.append(-1)
        ...
        sentences_stag = torch.LongTensor(sentences_stag)
        ...
        data = {
            ...
            'srctag_sentences':sentences_stag,
