...
class SequenceGenerator(object):
    ...
    def generate(self, src_lengths, encoded, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        scores_buf = scores.clone()  # [beam_size,max_len+1]
        tokens = src_lengths.new(bsz * beam_size, maxlen + 2).fill_(self.pad)
        tokens_buf = tokens.clone()  # [beam_size,max_len+2]
        tokens[:, 0] = self.bos
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            probs = self._decode(tokens[:, :step + 1].t(), encoded, incremental_state)
            if step == 0:
    ...
    def _decode(self, tokens, encoded, incremental_state):
        decoder_out = self.decoder(encoded, tokens, incremental_state=incremental_state)
        decoder_out = decoder_out[-1, :, :]  # T x B x C -> B x C  [batch,beam,vocab_size] -> [batch*beam,vocab_size]
